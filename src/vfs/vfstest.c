...
static bool run_test(struct vfstest_info *info, const struct test_struct suite[], size_t suite_size)
{
	...
	for (i = 0; i < suite_size; i++) {
		 ...
		 */
		if (t->require_fs_allow_idmap && !info->t_fs_allow_idmap) {
...
int main(int argc, char *argv[])
{
	...
	if (test_core) {
		if (!run_test(&info, basic_suite, ARRAY_SIZE(basic_suite)))
			goto out;

		if (!run_test(&info, idmapped_mounts.tests, idmapped_mounts.nr_tests))
			goto out;
	}

	if (test_fscaps_regression &&
	    !run_test(&info, fscaps_in_ancestor_userns.tests,
		      fscaps_in_ancestor_userns.nr_tests))
		goto out;
